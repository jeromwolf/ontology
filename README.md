...
- **📚 온톨로지**: 16개 챕터 + 3D 지식그래프 시뮬레이터
- **🔗 RAG 시스템**: 6개 챕터 + 6개 인터랙티브 시뮬레이터 (GraphRAG 포함)
- **💎 주식투자분석**: **23개 챕터 + 13개 핵심 시뮬레이터** + **실시간 데이터 API 준비**
...
- **📊 데이터 사이언스**: 12개 챕터 + 10개 시뮬레이터 (2025.08.11 전체 완료)
...
> **중요**: `kss-fresh`가 현재 메인 작업 디렉토리입니다. `kss-standalone`은 백업용으로 보관됩니다.
...
### Session 26 - GraphRAG 탐색기 문서 처리 기능 수정 🔍 (2025-08-10)

#### 🛠️ 문제 해결: 더미 그래프 → 실제 문서 분석
- **기존 문제**: GraphRAG 탐색기가 사용자 입력 문서를 무시하고 하드코딩된 더미 데이터만 표시
- **해결 방법**: 실제 텍스트 분석 기반 엔티티/관계 추출 시스템 구현
- **결과**: 사용자가 입력한 문서에서 동적으로 지식 그래프 생성

#### 🧠 스마트 텍스트 분석 엔진 구현
- **엔티티 추출**: 정규식 패턴 기반으로 5가지 타입 자동 감지
  - 조직/기업: KSS, Apple, 삼성전자, Google, Tesla 등
  - 인물: 스티브 잡스, 팀 쿡, 일론 머스크, 젠슨 황 등
  - 장소: 실리콘밸리, 서울, 쿠퍼티노, 캘리포니아 등
  - 개념/기술: AI, 온톨로지, 블록체인, 머신러닝, VR/AR 등
- **관계 추출**: 한국어 문법 패턴 분석으로 엔티티 간 관계 자동 추출
  - "A가 B를 개발", "A와 B의 협력", "A의 CEO", "A는 B에 위치" 등

#### 🎨 동적 그래프 시각화 개선
- **커뮤니티 감지**: 엔티티 타입별 자동 그룹핑 + 연결도 기반 클러스터링
- **색상 시스템**: 조직(파랑), 인물(초록), 장소(노랑), 개념(보라), 이벤트(빨강)
- **인터랙티브 쿼리**: 추출된 엔티티 기반 실시간 질의응답 시스템

#### 🚀 향상된 사용자 경험
- **실시간 분석**: 문서 입력 → 1.5초 후 맞춤형 지식 그래프 생성
- **지능형 쿼리**: "KSS가 뭐야?", "관계는?", "개수는?" 등 자연어 질문 지원
- **시각적 피드백**: 노드 크기는 연결도에 비례, 호버/클릭 효과로 관계 하이라이트

#### 💡 기술적 구현
- **패턴 매칭**: 15+ 조직, 10+ 인물, 8+ 장소, 12+ 기술 개념 패턴
- **관계 패턴**: 5가지 한국어 문법 패턴으로 동사 기반 관계 추출
- **폴백 시스템**: 엔티티 부족 시 기본 분석 엔티티 자동 생성
- **성능 최적화**: 엔티티 15개, 관계 20개 제한으로 렌더링 성능 보장

### Session 25 - AI 뉴스 분석 시뮬레이터 확장 📰 (2025-08-10)

#### 🎯 주식투자 모듈 AI 뉴스 분석 도구 3종 완성
- **기존 1개 → 3개로 확장**: 다양한 관점의 뉴스 분석 도구 제공
  1. **AI 뉴스 분석기**: 감정분석 & 주가 영향도 예측
  2. **AI 뉴스 온톨로지 분석기**: 기업 관계 & 지식그래프 시각화
  3. **뉴스 API 캐시 대시보드**: 실시간 데이터 관리 & 성능 모니터링

#### 📊 AI 뉴스 온톨로지 분석기 주요 기능
- **실시간 뉴스 API 연동**: NewsAPI 통합으로 최신 뉴스 자동 수집
- **엔티티 자동 추출**: 기업명, 섹터, 키워드 AI 분석
- **온톨로지 그래프 시각화**: D3.js 기반 인터랙티브 관계도
- **다중 필터링**: 날짜, 감정도, 섹터별 뉴스 필터
- **트렌딩 토픽**: 실시간 인기 검색어 제공

#### 🔧 기술적 구현
- **API 통합**: `/api/news/fetch` 엔드포인트로 뉴스 데이터 수집
- **캐시 시스템**: NewsManager로 API 호출 최적화
- **목업 데이터**: API 키 없을 때도 현실적인 최신 뉴스 제공
- **에러 핸들링**: API 한도/네트워크 오류 시 자동 폴백

#### 💼 실무 활용 사례
- **투자 의사결정**: 뉴스가 주가에 미치는 영향 실시간 분석
- **기업 관계 파악**: 경쟁사, 협력사, 공급망 관계 시각화
- **시장 트렌드**: 섹터별 뉴스 동향 모니터링
- **리스크 관리**: 부정적 뉴스 조기 감지
...
- **Unsplash Integration**: 무료 고품질 스톡 이미지 검색 & 다운로드
...
### 데이터 사이언스 모듈 대규모 개선
- **Chapter 리팩토링 완료**: 
  - Chapter5.tsx: 1,178줄 → 25줄 (97.9% 감소)
  - Chapter6.tsx: 1,186줄 → 26줄 (97.8% 감소)
  - 각 챕터를 5개 섹션으로 분리하여 관리성 향상
- **새로운 모듈 메인 페이지**:
  - 3가지 뷰 모드: 개요, 챕터 학습, 시뮬레이터
  - 난이도별 학습 경로 (초급/중급/고급)
  - 직접 시뮬레이터 접근 가능
  - 학습 진행률 시각화
- **10개 시뮬레이터 구조 생성**:
  - ML Playground (머신러닝 알고리즘 시각화) - 구현 중
  - Statistical Lab, Neural Network Builder 등 9개 추가 예정
- **UX 개선 사항**:
  - 모듈 진입 시 학습 경로 선택 가능
  - 시뮬레이터 그리드 뷰로 빠른 접근
  - 탭 네비게이션으로 편리한 전환
...
### 프로젝트 구조 개선
- **모듈 리팩토링 완료**: Smart Factory, LLM, Ontology, Data Science
- **파일 크기 관리**: 모든 챕터 파일 500줄 이하 유지
- **동적 임포트 적용**: 성능 최적화를 위한 lazy loading
