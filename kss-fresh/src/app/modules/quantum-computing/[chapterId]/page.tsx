...
export default function ChapterPage({ params }: ChapterPageProps) {
  ...
  const markAsCompleted = () => {
    ...
    setIsCompleted(true)
    localStorage.setItem('quantum-computing-progress', JSON.stringify(newProgress))
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {/* Chapter Header */}
      <div className="bg-gradient-to-br from-purple-500 to-violet-600 rounded-2xl p-8 mb-8 text-white">
      ...
      {/* Completion Button */}
      {!isCompleted && (
        <div className="bg-gradient-to-r from-purple-50 to-violet-50 dark:from-gray-800 dark:to-gray-800 rounded-xl p-6 mb-8 text-center">
          <button
            onClick={markAsCompleted}
            className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-violet-600 text-white px-8 py-3 rounded-lg font-semibold hover:from-purple-700 hover:to-violet-700 transition-all transform hover:scale-105"
          >
            <CheckCircle className="w-5 h-5" />
            챕터 완료 표시
          </button>
      ...
      <div className="flex justify-between items-center">
        <div>
          {previousChapter && (
            <Link
              href={`/modules/quantum-computing/${previousChapter.id}`}
        ...
        <div>
          {nextChapter && (
            <Link
              href={`/modules/quantum-computing/${nextChapter.id}`}
              className="inline-flex items-center gap-2 text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 font-medium"
            >
