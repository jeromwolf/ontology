          ...
          <div className="flex border-b border-gray-200 dark:border-gray-700">
            {tabs.map((tab) => (
              ...
              >
                ...
                {activeTab === tab.id && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-pink-500 to-red-500" />
                )}
              </button>
            ))}
          </div>
                    ...
                    <div className="grid gap-4">
                      {moduleMetadata.chapters.map((chapter, index) => (
                        <button
                          ...
                          className="bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg shadow p-6 text-left transition-all border-2 border-transparent hover:border-pink-500"
                        >
                          <div className="flex items-start gap-4">
                            <div className="flex-shrink-0 w-10 h-10 bg-gradient-to-r from-pink-500 to-red-500 rounded-lg flex items-center justify-center text-white font-bold">
                              {index + 1}
                            </div>
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {moduleMetadata.simulators.map((simulator) => (
                    <Link
                      key={simulator.id}
                      href={`/modules/medical-ai/simulators/${simulator.id}`}
                      className="bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg shadow p-6 transition-all border-2 border-transparent hover:border-pink-500 group"
                    >
                      <div className="flex items-start gap-4">
                        <div className="flex-shrink-0 w-12 h-12 bg-gradient-to-r from-pink-500 to-red-500 rounded-lg flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">
                          🏥
                        </div>
                        <div className="flex-1">
                          <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white group-hover:text-pink-600 dark:group-hover:text-pink-400">
                            {simulator.title}
                          </h3>
