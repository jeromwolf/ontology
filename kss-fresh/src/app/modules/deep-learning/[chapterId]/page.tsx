...
export default function DeepLearningChapterPage() {
  const params = useParams()
  const router = useRouter()
  const chapterId = params.chapterId as string

  const currentChapterIndex = deepLearningModule.chapters.findIndex(ch => ch.id === chapterId)
  const currentChapter = deepLearningModule.chapters[currentChapterIndex]
  const nextChapter = deepLearningModule.chapters[currentChapterIndex + 1]
  const prevChapter = deepLearningModule.chapters[currentChapterIndex - 1]

  if (!currentChapter) {
    router.push('/modules/deep-learning')
    return null
  }
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-4">
          <Link href="/modules/deep-learning" className="hover:text-violet-600 dark:hover:text-violet-400">
            딥러닝 모듈
          </Link>
          <span>/</span>
      ...
      {currentChapter.learningObjectives && (
        <div className="bg-violet-50 dark:bg-violet-900/20 rounded-xl p-6 mb-8">
      ...
      <div className="flex justify-between items-center mt-12 pt-8 border-t border-gray-200 dark:border-gray-700">
        {prevChapter ? (
          <Link
            href={`/modules/deep-learning/${prevChapter.id}`}
        ...
        {nextChapter ? (
          <Link
            href={`/modules/deep-learning/${nextChapter.id}`}
            className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-violet-600 dark:hover:text-violet-400 transition-colors"
          >
          ...
          <Link
            ...
            className="flex items-center gap-2 text-violet-600 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-500 transition-colors"
          >
            <div className="text-right">
