...
export default function ChapterPage({ params }: { params: { chapterId: string } }) {
  ...
  const markAsComplete = () => {
    ...
    localStorage.setItem('ai-automation-progress', JSON.stringify(progress))
    setCompleted(true)
  }
  ...
  if (!chapter) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/modules/ai-automation" className="text-violet-600 dark:text-violet-400 hover:underline">
          모듈 홈으로 돌아가기
        </Link>
      </div>
    )
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-8">
        <Link href="/modules/ai-automation" className="hover:text-violet-600 dark:hover:text-violet-400">
          AI 자동화 도구
        </Link>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 mb-8 border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl flex items-center justify-center text-white font-bold text-2xl">
              {chapter.id}
            </div>
            <div>
        ...
        {/* Learning Objectives */}
        <div className="mt-6 p-6 bg-violet-50 dark:bg-violet-900/20 rounded-xl">
      ...
      <div className="flex items-center justify-between mb-8">
        {prevChapter ? (
          <Link
            href={`/modules/ai-automation/${prevChapter.id}`}
        ...
        {!completed && (
          <button
            ...
            className="px-6 py-3 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl hover:from-violet-700 hover:to-purple-700 transition-colors font-semibold"
          >
            학습 완료
          </button>
        )}
        ...
        {nextChapter ? (
          <Link
            href={`/modules/ai-automation/${nextChapter.id}`}
